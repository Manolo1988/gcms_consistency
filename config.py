...
class Config:
    ...
    model_select_pseudo_unknown_products: str = "H88,H99,HRJ,HRY,HYZ,RJD,YJD"
    model_select_pseudo_unknown_min_samples: int = 5
    model_select_pseudo_unknown_max_products: int = 7
    model_select_save_checkpoints: bool = False
    ...
    lambda_adv: float = 0.25                  # λ₁ 批次对抗 (加强去批次)
    lambda_proto: float = 0.85                # λ₂ 原型紧凑 (类内紧凑)
    lambda_recon: float = 0.25                # 重建正则
    lambda_cls: float = 0.40                  # Focal 分类辅助头权重
    lambda_hard_pair: float = 0.12            # 易混产品对边界约束 (加强)
    ...
    open_score_base_weight: float = 0.0       # 开集分数: base score 权重 (训练-评估对齐: 仅用margin)
    open_score_margin_weight: float = 1.0     # 开集分数: 最近邻间隔权重
    ...
    open_score_calibration_enabled: bool = True   # 启用开集分数校准(Logistic Regression)
    open_score_calibration_mode: str = "logistic"  # pseudo_unknown / grid_search / logistic
    open_score_features: str = "base_score,margin_score,min_dist,radius_norm,second_dist"
